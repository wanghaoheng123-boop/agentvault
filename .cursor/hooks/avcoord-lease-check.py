...
# The ONE verb that must be reachable with zero leases. Without it the lease
# system cannot be ENTERED: the escape hatch below requires a lease already
# held, so a fresh agent could never obtain its first one — and the denial
# message told it to run the very command being denied. Found in the field: a
# task was stranded because its agent could not enter the lease system at all.
#
# Safe to allowlist because `claim` carries its own authorization: cmd_claim
# refuses a resource overlapping a live lease held by another principal
# (`resources_overlap` -> return 1), verified live. `release` is deliberately
# NOT here — unbound by a run token it can target another principal's leases —
# and the escape hatch already covers it once a lease is held.
SHELL_BOOTSTRAP = {"bin/avcoord": frozenset({"claim"})}
...
def shell_decision(payload: dict) -> int:
    ...
    if head in SHELL_BOOTSTRAP and len(argv) > 1 and argv[1] in SHELL_BOOTSTRAP[head]:
        return allow(f"bootstrap verb, reachable without a lease: {head} {argv[1]}",
                     code="AV_ALLOW_BOOTSTRAP_CLAIM")
    ...
    return deny(
        ...
        f"`bin/avcoord claim` IS permitted without a lease, precisely so you can "
        f"enter: bin/avcoord claim --agent "
        f"{os.environ.get('AVCOORD_AGENT', 'orchestrator')} "
        f"--resource <the path you intend to write> -- then retry this command.",
        code="AV_DENY_SHELL_NOT_ALLOWLISTED")
...
def agent_holds_a_live_lease() -> bool:
    ...
    try:
        ...
        # Deliberately NOT gated on returncode. `status` exits nonzero when
        # CURRENT.md is stale or doctor reports issues -- neither says anything
        # about lease state. Treating that as "no lease" left the hatch dead in
        # every fresh install, whose CURRENT.md ships pre-stale. Fail-closed is
        # preserved by the parse: unreadable output still yields no leases.
        data = json.loads(proc.stdout or "{}")
    except (OSError, subprocess.TimeoutExpired, json.JSONDecodeError):
        return False
    # `status --json` emits the principal as "agent". Reading "agent_id" here
    # made this function return False unconditionally, which silently disabled
    # the escape hatch entirely. Accept both spellings so a schema change
    # cannot re-disable enforcement's release valve without a test failing.
    for lease in data.get("leases") or []:
        if not isinstance(lease, dict):
            continue
        if agent in (lease.get("agent"), lease.get("agent_id")):
            return True
    return False
